...
--				and feedback answers (numeric ones)
--			    
-- History:		v1
-- =============================================
CREATE PROCEDURE [dbo].[export_feedback_num]
...
SELECT     
  ...
  MAX(CASE WHEN (m.fbnum=1)  then meas_n.[Value]  else null END) as fx1,
  MAX(CASE WHEN (m.fbnum=2)  then meas_n.[Value]  else null END) as fx2,
  MAX(CASE WHEN (m.fbnum=3)  then meas_n.[Value]  else null END) as fx3,
  MAX(CASE WHEN (m.fbnum=4)  then meas_n.[Value]  else null END) as fx4,
  MAX(CASE WHEN (m.fbnum=5)  then meas_n.[Value]  else null END) as fx5,
  MAX(CASE WHEN (m.fbnum=6)  then meas_n.[Value]  else null END) as fx6,
  MAX(CASE WHEN (m.fbnum=7)  then meas_n.[Value]  else null END) as fx7,
  MAX(CASE WHEN (m.fbnum=8)  then meas_n.[Value]  else null END) as fx8,
  MAX(CASE WHEN (m.fbnum=9)  then meas_n.[Value]  else null END) as fx9,
  MAX(CASE WHEN (m.fbnum=10)  then meas_n.[Value]  else null END) as fx10,
  MAX(CASE WHEN (m.fbnum=11)  then meas_n.[Value]  else null END) as fx11,
  MAX(CASE WHEN (m.fbnum=12)  then meas_n.[Value]  else null END) as fx12,
  MAX(CASE WHEN (m.fbnum=13)  then meas_n.[Value]  else null END) as fx13,
  MAX(CASE WHEN (m.fbnum=14)  then meas_n.[Value]  else null END) as fx14,
  MAX(CASE WHEN (m.fbnum=15)  then meas_n.[Value]  else null END) as fx15,
  MAX(CASE WHEN (m.fbnum=16)  then meas_n.[Value]  else null END) as fx16
...
FROM         #fromview as xr left JOIN
                      Applicant_Measures_Num as meas_n ON
                       ...
                       (xr.s = meas_n.Session_ID) left join
                        dbo.get_feedback_measure_function(@Assessment_ID, 0, @cv) as m
